# -*- coding: utf-8 -*-
"""通过本地 Patchright 浏览器执行 ChatGPT 注册（免费，无需 license/API Key）。"""
...
# 复用 Roxy/Cloak 注册流程里已维护好的页面操作函数。
...
def run_patchright_registration(
    ...
    try:
        ...
        try:
            traffic_tracker = PlaywrightTrafficTracker(driver.context, label="Patchright")
        except Exception as exc:
            # 统计失败不应影响注册主流程。
            logger.warning("[Patchright注册] 初始化浏览器流量统计失败，继续注册：%s: %s", type(exc).__name__, str(exc)[:180])
        data_saver = BrowserDataSaver(label="Patchright")
        if traffic_tracker is not None:
            traffic_tracker.attach_data_saver(data_saver)
        data_saver.install_playwright(driver.context)
        logger.info("[Patchright注册] 开始：%s，profile=%s", email, opened.profile_id)

        otp_after_ts = time.time()
        logger.info("[Patchright注册] 打开登录页：https://chatgpt.com/auth/login")
        ...
        for otp_attempt in range(1, max_otp_attempts + 1):
            if current_otp is None:
                logger.info("[Patchright注册][OTP] 等待验证码：%s（第 %s/%s 次）", email, otp_attempt, max_otp_attempts)
                try:
                    ...
                    if otp_attempt >= max_otp_attempts:
                        raise
                    logger.warning(
                        "[Patchright注册][OTP] 一直未收到验证码，点击“重新发送电子邮件”后继续等待（下一轮 %s/%s）：%s: %s",
                        otp_attempt + 1,
                        max_otp_attempts,
                        type(exc).__name__,
                        str(exc)[:180],
                    ...
                    current_otp = None
                    continue
            logger.info("[Patchright注册][OTP] 收到验证码：%s", current_otp)
            ...
            try:
                ...
                logger.info("[Patchright注册][OTP] 未找到显式提交按钮，继续等待页面状态：%s", str(exc)[:120])
        ...
        logger.info("[Patchright注册] 已拿到 accessToken：%s", email)

        if _twofa_cfg.ENABLE_2FA:
            logger.warning("[Patchright注册] 当前 Patchright 自动化路径暂不执行 2FA 设置，已跳过")
        totp_secret = None
        ...
        try:
            ...
            if bool(getattr(_codex_cfg, "ENABLE_CODEX_AUTO", False)):
                ...
                logger.info("[Patchright注册][Codex] ENABLE_CODEX_AUTO=True，复用当前浏览器窗口执行 Codex 授权")
                _check_manual_stop()
                ...
                logger.info("[Patchright注册][Codex] ENABLE_CODEX_AUTO=False，注册后跳过 Codex OAuth")
        except Exception as exc:
            codex_result = {"status": "failed", "ok": False, "message": f"{type(exc).__name__}: {str(exc)[:180]}"}

        # 统计注册浏览器关闭前的完整会话；注册后停留期间的网络请求也计入。
        post_register_dwell(email, label="Patchright注册")
        if traffic_tracker is not None:
        ...
        account_id = save_account_data(
            ...
            extra={
                ...
                "patchright": {"profile_id": opened.profile_id, "open_result": opened.raw},
        ...
        if data_saver is not None:
            data_saver.stop()
        logger.error("[Patchright注册] 失败：%s: %s", type(exc).__name__, exc)
        logger.debug("[Patchright注册] 失败详情", exc_info=True)
        try:
            if email:
                from core.email_provider import release_email
                release_email(email, status="failed" if create_acknowledged else "available", note=f"Patchright注册失败: {str(exc)[:180]}")
        except Exception:
            pass
        return {
            "success": False,
        ...
        if data_saver is not None:
            data_saver.stop()
        if driver and not bool(_cfg.PATCHRIGHT_KEEP_BROWSER_OPEN):
            try:
                driver.quit()
            except Exception:
                pass
